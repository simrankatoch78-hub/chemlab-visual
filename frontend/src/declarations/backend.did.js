/* eslint-disable */

// @ts-nocheck

// This file was automatically generated by @caffeinelabs/bindgen@0.1.0.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { IDL } from '@icp-sdk/core/candid';

export const Reaction = IDL.Record({
  'description' : IDL.Text,
  'reactionType' : IDL.Text,
  'visualEffect' : IDL.Text,
  'products' : IDL.Vec(IDL.Text),
  'equation' : IDL.Text,
  'reactants' : IDL.Vec(IDL.Text),
});

export const idlService = IDL.Service({
  'getAllReactions' : IDL.Func([], [IDL.Vec(Reaction)], ['query']),
  'getReactionByName' : IDL.Func(
      [IDL.Text],
      [
        IDL.Variant({
          'ok' : IDL.Opt(Reaction),
          'contentBlocked' : IDL.Opt(IDL.Text),
        }),
      ],
      ['query'],
    ),
  'searchReactions' : IDL.Func([IDL.Text], [IDL.Vec(Reaction)], ['query']),
});

export const idlInitArgs = [];

export const idlFactory = ({ IDL }) => {
  const Reaction = IDL.Record({
    'description' : IDL.Text,
    'reactionType' : IDL.Text,
    'visualEffect' : IDL.Text,
    'products' : IDL.Vec(IDL.Text),
    'equation' : IDL.Text,
    'reactants' : IDL.Vec(IDL.Text),
  });
  
  return IDL.Service({
    'getAllReactions' : IDL.Func([], [IDL.Vec(Reaction)], ['query']),
    'getReactionByName' : IDL.Func(
        [IDL.Text],
        [
          IDL.Variant({
            'ok' : IDL.Opt(Reaction),
            'contentBlocked' : IDL.Opt(IDL.Text),
          }),
        ],
        ['query'],
      ),
    'searchReactions' : IDL.Func([IDL.Text], [IDL.Vec(Reaction)], ['query']),
  });
};

export const init = ({ IDL }) => { return []; };